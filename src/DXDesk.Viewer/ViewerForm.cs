...
public sealed class ViewerForm : Form
{
    ...
    private readonly DXDesk.Core.FileTransferReceiver _fileRecv;
    ...
    public ViewerForm(string title)
    {
        ...
        var clipStage = System.IO.Path.Combine(System.IO.Path.GetTempPath(), "DXDesk", "clip");
        _fileRecv = new DXDesk.Core.FileTransferReceiver(SetClipboardFiles, clipStage, VLog);
    ...
    /// <summary>Viewer (admin) tərəf diaqnostika loqu: %TEMP%\DXDesk-viewer.log</summary>
    private static void VLog(string m)
    {
        try
        {
            System.IO.File.AppendAllText(
                System.IO.Path.Combine(System.IO.Path.GetTempPath(), "DXDesk-viewer.log"),
    ...
    private void PollLocalClipboard()
    {
        ...
        try
        {
            ...
            if (Clipboard.ContainsFileDropList())
            {
                ...
                col.CopyTo(files, 0);
                var sig = DXDesk.Core.FileTransfer.Signature(files);
                if (sig == _lastClipSig) return;
                _lastClipSig = sig;
                VLog($"admin clipboard-da {files.Length} fayl -> uzaq PC-yə göndərilir");
                foreach (var frame in DXDesk.Core.FileTransfer.Build(files, (byte)InputType.File))
                {
                    Raise(frame);
                }
                return;
    ...
    public void HandleFileFrame(byte[] sub)
    {
        ...
        if (sub.Length >= 1 && sub[0] == DXDesk.Core.FileTransfer.SubCommit && _fileRecv.LastSignature is not null)
        {
