...
app.MapGet("/", () => Results.Text("MillaRemote Relay işləyir. Hub: /remotehub", "text/plain"));
app.MapGet("/health", () => Results.Ok(new { status = "ok", time = DateTimeOffset.UtcNow }));
app.MapGet("/api/clients", (ClientRegistry registry) => Results.Ok(registry.GetClients()));

app.Logger.LogInformation("MillaRemote Relay başladı. Hub endpoint: /remotehub");
