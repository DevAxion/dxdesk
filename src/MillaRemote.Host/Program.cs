...
static async Task SelfTestAsync(CancellationToken ct)
{
    ...
    using var decoder = new MillaRemote.Core.TileFrameDecoder();
    ...
    try
    {
        await receiver.ReceiveLoopAsync(payload =>
        {
            ...
            if (payload.Length > 0 && payload[0] == MillaRemote.Core.TileScreenEncoder.KindKeyFrame) keyframes++;
...
{
    using var encoder = new MillaRemote.Core.TileScreenEncoder(tileSize: 128);
    using var decoder = new MillaRemote.Core.TileFrameDecoder();
    ...
    for (var i = 0; i < 20 && !ct.IsCancellationRequested; i++)
    {
        ...
        else
        {
            frames++;
            var kind = payload[0];
            if (kind == MillaRemote.Core.TileScreenEncoder.KindKeyFrame)
            {
