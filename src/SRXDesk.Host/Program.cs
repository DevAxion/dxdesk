...
static async Task SelfTestAsync(CancellationToken ct)
{
    ...
    using var decoder = new SRXDesk.Core.TileFrameDecoder();
    ...
    try
    {
        await receiver.ReceiveLoopAsync(payload =>
        {
            ...
            if (payload.Length > 0 && payload[0] == SRXDesk.Core.TileScreenEncoder.KindKeyFrame) keyframes++;
...
{
    using var encoder = new SRXDesk.Core.TileScreenEncoder(tileSize: 128);
    using var decoder = new SRXDesk.Core.TileFrameDecoder();
    ...
    for (var i = 0; i < 20 && !ct.IsCancellationRequested; i++)
    {
        ...
        else
        {
            frames++;
            var kind = payload[0];
            if (kind == SRXDesk.Core.TileScreenEncoder.KindKeyFrame)
            {
